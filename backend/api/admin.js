/**
 * backend/api/admin.js — Khu vực quản trị (yêu cầu role admin)
...
router.get('/stats', async (_req, res) => {
  try {
    const [[u]] = await pool.query(`SELECT COUNT(*) AS n FROM tai_khoan WHERE loai_tai_khoan = 'khach_hang'`);
...
router.get('/users', async (_req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT kh.id, kh.ho_ten, tk.email, kh.so_dien_thoai, tk.trang_thai, tk.created_at
         FROM khach_hang kh
  ...
  try {
    const [rows] = await pool.query(
      `SELECT dh.id, dh.ma_don_hang, dh.tong_thanh_toan, dh.trang_thai_don_hang, dh.created_at,
              kh.ho_ten AS khach_hang, tk.email AS khach_email
         FROM don_hang dh
