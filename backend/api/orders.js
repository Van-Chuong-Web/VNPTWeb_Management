/**
 * backend/api/orders.js — Đơn hàng (checkout)
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const tongTienHang = lines.reduce((s, l) => s + Number(l.don_gia_tai_thoi_diem) * Math.min(l.so_luong, 1), 0);
    const giamGia = 0;
    ...
    const [orderRes] = await conn.query(
      `INSERT INTO don_hang
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'cho_xac_nhan', ?)`,
      [maDon, khId, tongTienHang, phiVanChuyen, giamGia, tongThanhToan, ghiChu]
    );
